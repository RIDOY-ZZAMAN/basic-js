...
var monCalculation = seerToMon(40);
...
//Calling the function and assigned to a variable
const overAllDeliveryCost = deliveryCost(120);
...
function perfectFriend(names) {
    //For loop, for go through the array
    for (var name of names) {
        //Checking the name, which length is equal to 5
        if (name.length == 5) {
            break;
        }
        //If the user insert number {For example [40] } instead of string in the "names" array, then the system will show this string type error message.
        else if (typeof name == "number") {
            return "Please Insert strings in your [names] Array, not a number";
        }
    }
    //returning the name, which length is equal to 5;
    return name;
}
//Array of my friends name
const names = ['MD sadi Ifti', 'Ridoy Nur', 'Rahman Sohan', 'Nayim', 'Ashraf Durjoy', 'Mamun', 'MD Rayhan'];
//Calling the function and assigned to a variable
const myPerfectFriend = perfectFriend(names);
//Finally,we see the output by console log
console.log(myPerfectFriend);
    //---------------------------Answer to the Problem 4 End----------------------------